 ...
 */
public class MCSExecutorAdvanced {
	...
	public double simulate(int n) {
		...
		MyList<Runnable> rejectedRunnables = new MyList<Runnable>();
		ExecutorService executor = Executors.unconfigurableExecutorService(
				new ThreadPoolExecutor(threadCount/2, threadCount/2, 0L, TimeUnit.MILLISECONDS, 
						new LinkedBlockingDeque<Runnable>(threadCount/2), new MyRejectedExecutionHandler(rejectedRunnables)));
		
		long startTime = System.currentTimeMillis();
		for (Runnable calculatePart : calculatePartList) {
			executor.execute(calculatePart);
		}
		while (!rejectedRunnables.isEmpty()) {
			MyList<Runnable> tempList = new MyList<Runnable>(rejectedRunnables);
			rejectedRunnables.clear();
			for (Runnable calculatePart : tempList) {
				executor.execute(calculatePart);
			}
		}
		executor.shutdown();
	...
	public static void main(String[] args) {
		...
		MCSExecutorAdvanced mcs = new MCSExecutorAdvanced();
